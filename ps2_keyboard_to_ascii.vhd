...
ARCHITECTURE behavior OF ps2_keyboard_to_ascii IS
  TYPE machine IS(ready, new_code, translate, output,new_ps2_key);              --needed states
  ...
  SIGNAL ps2_code_trans    : STD_LOGIC_VECTOR(7 DOWNTO 0);          --PS2 code input to be sent for translation to ASCII
  SIGNAL ps2_key_new       : STD_LOGIC;						           --PS2 key state from ps2_key input
  SIGNAL ps2_key_prev      : STD_LOGIC;						           --PS2 key previous state from ps2_key input
...
BEGIN
  ...
  BEGIN
    IF(clk'EVENT AND clk = '1') THEN
	   --code for direct input from ps2_key port
		ps2_key_prev<=ps2_key_new;
		ps2_key_new<=ps2_key(10);
		
		--code for input from ps2 data & clk lines
      ...
      CASE state IS
        ...
        WHEN ready =>
          ...
          ELSIF (ps2_key_prev/=ps2_key_new) THEN			 --or new PS2 key received
				ascii_new<= '0';
				state<=new_ps2_key;
			 ELSE
        ...
        WHEN new_code =>
          ...
          ELSE                         --code is the last PS2 code in the make/break code
            ascii(7) <= '1';             --set internal ascii value to unsupported code (for verification)
				ps2_code_trans<=ps2_code;
            state <= translate;          --proceed to translate state
				--state<=ready;
          END IF;
			 
			 --new_ps2_key state: determine what to do with the new PS2 key code  
        WHEN new_ps2_key =>
          IF(ps2_key(9) = '0') THEN
				break <= '1';   --code indicates that next command is break so set break flag
			 END IF;	
                            
          IF(ps2_key(8) = '1') THEN
				e0_code <= '1'; --code indicates multi-key command so set multi-code command flag
			 END IF;
            
												     --code is the last PS2 code in the make/break code
          ascii(7) <= '1';             --set internal ascii value to unsupported code (for verification)
			 ps2_code_trans<=ps2_key(7 DOWNTO 0);
          state <= translate;          --proceed to translate state
				--state<=ready;
